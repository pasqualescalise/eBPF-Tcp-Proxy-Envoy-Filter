...
Network::FilterStatus EbpfTcpProxySKB::onData(Buffer::Instance& data, bool end_stream) {
  ...
  if (upstream_) {
    ...
    upstream_->encodeData(data, false);
  }
 ...
 */
void EbpfTcpProxySKB::bindClientAndServerConnections() {
  ...
  int err = bpf_map_update_elem(connection_fingerprint_to_connection_fingerprint_map_fd,
                                &client_fingerprint, &server_fingerprint, BPF_ANY);
  if (err < 0) {
  ...
  err = bpf_map_update_elem(connection_fingerprint_to_connection_fingerprint_map_fd,
                            &server_fingerprint, &client_fingerprint, BPF_ANY);
  if (err < 0) {
